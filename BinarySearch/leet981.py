# Time Based Key Value Store: Design a time-based key-value data structure that can store multiple values for the same key 
...
class Solution:
  ...
  def set(self, key: str, value: str, timestamp: int) -> None:
    if key not in self.store:
      self.store[key] = [] # setting the key to an empty list since its not present in store
  ...
  def get(self, key: str, timestamp: int) -> str:
    ...
    l, r = o, len(values) - 1
    while l <= r:
      m = (l + r) // 2 # 2 slash for integer division else python does decimal division
